...
class LdapServer:
    ...
    def add_user(self, domain=None, gid=None, passwd=None, uid=None, upasswd=None, user=None) -> None:
        ...
        if dcs and user and passwd and upasswd and uid and uid > MIN_UID and gid and gid > MIN_GID:
            binddn = f"cn=admin,{dcs}"
            base_user = [f"dn: uid={user.lower()},ou=people,{dcs}", "objectClass: inetOrgPerson",
    ...
    def add_group(self, domain=None, gid=None, group=None, passwd=None) -> None:
        ...
        if dcs and group and passwd and gid and gid > MIN_GID:
            binddn = f"cn=admin,{dcs}"
            base_group = [f"dn: cn={group},ou=Groups,{dcs}", "objectClass: posixGroup",
    ...
    def tls_deb(self, domain, org, passwd) -> None:
        ...
        args = [
            ...
            f"slapd shared/organization string {org}", f"slapd slapd/password1 password {passwd}",
    ...
    def tls_gen(self, org=None) -> None:
        ...
        if org:
            ...
            sc_template = [f"organization = {org}", f"cn = {result.stdout}", "tls_www_server", "encryption_key", "signing_key", "expiration_days = 365"]
    ...
    def tls_load(self, domain=None):
        ...
        if domain:
            ...
            sssd_conf = [
                "[sssd]", "config_file_version = 2", f"domains = {domain}", "", f"[domain/{domain}]",
                "id_provider = ldap", "auth_provider = ldap", f"ldap_uri = ldap://{ldap_uri.stdout}",
                "cache_credentials = True", f"ldap_search_base = {dcs}"
            ]
